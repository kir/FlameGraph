#!/usr/bin/perl -w
#
# stackcollapse-tc.pl	collapse TeamCity-generated jstack samples into single lines.
#
# Parses Java stacks generated by TeamCity server thread dump generator and outputs RUNNABLE stacks as
...
#
# USAGE: ./stackcollapse-tc.pl infile > outfile
#
# Example input:
#
...
foreach (<>) {
	...
	if (m/^$/) {
		...
		$has_match = $has_match || $with_pattern eq "";
...
clear:
	...
	if (/^"([^"]*)/) {
		...
		$has_match = $has_match || index($_, $with_pattern) != -1;
